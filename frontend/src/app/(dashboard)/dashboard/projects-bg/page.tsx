...
export default function JobsBgPage() {
  ...
  const handleApplyBackground = async () => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        showMessage('✅ Fond d\'écran appliqué avec succès ! Visible sur la page Offres.', 'success');
      } else {
  ...
  const handleDelete = async () => {
    if (!confirm('⚠️ Supprimer définitivement le fond d\'écran de la page Offres ?')) return;
    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-800">Fond d'écran - Page Offres d'emploi</h1>
      ...
      <div className="bg-white rounded-xl shadow-md p-6 mb-6">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-semibold text-gray-800 flex items-center gap-2">
            <Eye className="w-5 h-5" /> Fond actuel - Page Offres
          </h2>
      ...
      {previewUrl && (
        <div className="bg-white rounded-xl shadow-md p-6 mb-6 border-2 border-blue-400">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-semibold text-gray-800">🔄 Aperçu de la nouvelle image</h2>
          ...
          <div className="mt-4 flex gap-3">
            <button
              ...
              disabled={uploading}
              className="flex-1 bg-blue-600 text-white py-2.5 rounded-lg font-semibold hover:bg-blue-700 transition disabled:opacity-50 flex items-center justify-center gap-2"
            >
              {uploading ? (
                <>
                ...
                <>
                  <CheckCircle className="w-5 h-5" /> Appliquer ce fond
                </>
              )}
            </button>
            <button onClick={cancelPreview} className="flex-1 border border-gray-300 py-2.5 rounded-lg hover:bg-gray-50 transition">
              Annuler
            </button>
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-md p-6">
        ...
        <div 
          className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center bg-gray-50 hover:border-blue-400 transition cursor-pointer"
        ...
        >
          ...
          <div className="flex flex-col items-center">
            ...
            <p className="text-xs text-gray-400 mt-2">
              JPEG, PNG, WebP • Max 10 Mo • 1920x1080 recommandé
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-6 bg-blue-50 rounded-xl p-4 border border-blue-200">
        <div className="flex items-center gap-2 text-blue-700 mb-2">
          ...
          <span className="text-sm font-medium">Aperçu de l'espace public</span>
        </div>
        <div className="relative h-32 rounded-lg overflow-hidden">
          {bgImage?.url ? (
            <>
              <img src={bgImage.url} alt="Aperçu public" className="w-full h-full object-cover" />
              <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                <span className="text-white font-bold text-lg">💼 Offres d'emploi</span>
              </div>
            </>
          ) : (
            <div className="w-full h-full bg-gradient-to-r from-blue-800 to-blue-900 flex items-center justify-center">
              <span className="text-white/80">Page Offres d'emploi</span>
            </div>
          )}
        </div>
        <p className="text-xs text-center text-gray-500 mt-2">
          Ce fond apparaîtra sur la page <span className="font-mono">/jobs</span>
        </p>
      </div>

      {/* Conseils */}
      <div className="mt-6 bg-blue-50 rounded-xl p-5">
        <h3 className="font-semibold text-blue-800 mb-2">💡 Conseils pour le fond d'écran</h3>
        <ul className="text-sm text-gray-600 space-y-1">
          ...
          <li>• Thème : emploi, recrutement, jeunesse professionnelle</li>
        </ul>
      </div>
    </div>
  );
