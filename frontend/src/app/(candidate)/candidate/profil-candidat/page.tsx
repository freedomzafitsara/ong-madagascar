...
export default function CandidateProfilePage() {
  ...
  }, [user, isAuthenticated, router, getText]);
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>): Promise<void> => {
    ...
    if (!validTypes.includes(file.type)) {
      toast.error(getText(
        'Format non supporte. Utilisez JPG, PNG, WEBP ou GIF',
        'Tsy ekena. Mampiasa JPG, PNG, WEBP na GIF'
      ));
      return;
    }
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-4 flex-wrap">
              ...
              <div className="flex items-center gap-1 text-sm text-gray-500">
                ...
                <span>
                  {getText('Derniere connexion', 'Niditra farany')}: {new Date().toLocaleDateString('fr-FR')} à {new Date().toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}
                </span>
              </div>
            </div>
            <button
